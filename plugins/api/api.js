    ...
    server.post('/', (req, res) => {
        ...
        if (body.hasOwnProperty('EUI') && body.hasOwnProperty('data') && body.hasOwnProperty('seqno') && body.hasOwnProperty('ts')) {
            ...
            if (config.sensor_whitelist.indexOf(body.EUI) === -1) {
                logger.warn('Sensor not whitelisted or has no EUI. ' + JSON.stringify(body));
                res.status(401).send();
                return;
            }
            ...
            if (body.hasOwnProperty('gws')) {
                ...
                database.getSensorPackage(body.EUI, body.seqno, body.ts)
                    .then((rows) => {
                        ...
                        if (rows.length === 0) {
                            ...
                            logger.info('Duplicate entry. ' + JSON.stringify(body));
                ...
                logger.info('Entry without gws entry. Ignoring. ' + JSON.stringify(body));
